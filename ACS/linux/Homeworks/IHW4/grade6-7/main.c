...
//you will see the help -- the key difference between programms - as we were asked, it is arguments of the command line
void print_help() {
    printf("What you can enter:\n");
    printf("  ./main -n <number_of_chatties> -t <working_time>\n");
    printf("Parametres:\n");
    printf("  -n <number_of_chatties> (%d-%d)\n", MIN_CHATTERS, MAX_CHATTERS);
    printf("  -t <working_time> (%d-%d)\n", MIN_RUN_TIME, MAX_RUN_TIME);
    printf("  -h <see the help window>\n");
    printf("Example::\n");
    printf("  ./main -n 3 -t 30\n");
}

int main(int argc, char* argv[])  //here they are - our arguments
{
    struct sigaction sa; //setting signal handlers
    sa.sa_handler = signal_handler; //clean the signal mask
